...
func TestGetParentAndUncles(t *testing.T) {
	...
	tree := buildTestTree()
	{
		...
		tt.AssertEquals(0, len(uncles), "amount of parent and uncles of '1' in '%s'", tree)
	}
	{
		uncles := GetParentAndUncles(tree.GetChildren()[0])
		tt.AssertEquals(1, len(uncles), "amount of parent and uncles of '2' in '%s'", tree)
...
func TestGetSelfAndSiblings(t *testing.T) {
	...
	tree := buildTestTree()
	{
		...
		tt.AssertEquals(1, len(siblings), "amount of self and siblings of '1' in '%s'", tree)
...
func TestGetSelfSiblingsAndCousins(t *testing.T) {
	...
	tree := buildTestTree()
	{
		...
		tt.AssertEquals(1, len(cousins), "amount of self, siblings, and cousins of '1' in '%s'", tree)
		tt.AssertEquals(int64(1), cousins[0].GetValue().Int(), "first cousin")
	}
	{
		cousins := GetSelfSiblingsAndCousins(tree.GetChildren()[0])
		tt.AssertEquals(2, len(cousins), "amount of self, siblings, and cousins of '2' in '%s'", tree)
...
func TestGetChildrenAndNephews(t *testing.T) {
	...
	tree := buildTestTree()
	{
		...
		tt.AssertEquals(2, len(nephews), "amount of children and nephews of '1' in '%s'", tree)
